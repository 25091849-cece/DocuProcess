  ...
  <div class="table-wrap">
    <table>
      ...
      {% for doc in docs %}
        ...
        <tr class="doc-row"
            data-search="{{ (doc.documentId~' '~doc.vendor~' '~doc.date~' '~doc.amount~' '~doc.s3Key~' '~doc.status~' '~doc.confidence~' '~doc.get('rejectReason', ''))|lower }}">
...
function filterTable(q) {
  ...
  rows.forEach(row => {
    ...
    // Always restore original text first, then highlight if matching
  ...
  if (q) { 
    rc.textContent = `Found ${vis} document(s) matching "${q}"`; 
...
def delete_document(doc_id):
    try:
        ...
        if not s3_key:
            return jsonify({'success': False, 'error': 'S3 key not found in document'}), 400
        ...
        try:
            ...
            return jsonify({'success': False, 'error': f'S3 deletion failed: {str(s3_err)}'}), 400
        
        # Delete from DynamoDB
        try:
            table.delete_item(Key={'documentId': doc_id})
        except ClientError as db_err:
            print(f"DynamoDB delete error: {db_err}")
            return jsonify({'success': False, 'error': f'Database deletion failed: {str(db_err)}'}), 400
